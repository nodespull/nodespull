...
export class DB_Model_Rel_FilesRunner extends FilesEngine{
    ...
    constructor(public args?:DB_FilesRunner_option){
        super()
        if(args && DB_Controller.connections[args.dbConnectionSelector].migration.isRunning) {
            let targetFolderPath:string|undefined
            if(DB_Controller.connections[args.dbConnectionSelector].migration.isRevertMode) targetFolderPath = this.getFolderPath(PathVar.dbModule+"/SQL", "stage.v") // migration down scripts are in this folder
            else targetFolderPath = this.getFolderPath(PathVar.dbModule+"/SQL", "at.v") // migration up are here

            if(!targetFolderPath) 
                new Log(`missing folder with prefix '${DB_Controller.connections[args.dbConnectionSelector].migration.isRevertMode?
        ...
        if(args && DB_Controller.connections[args.dbConnectionSelector].migration.isRunning && args.overwrite_newStageScripts) this.updateStageFiles()
        else{
            super.recursiveSearch(PathVar.dbModule+"/SQL", "attribute.js", {runFiles:true});
    ...
    */
    updateStageFiles(){
        let modelPaths = super.recursiveSearch(PathVar.dbModule+"/SQL/stage.v"+(getCurrentDBVersion()+1)+"/","attribute.js",{runFiles:false})
